...
#rotate node between Hodgesia cytopus and Ficalbia ichiromiyagii
tree.analysis<-rotateNodes(tree.analysis, findMRCA(tree.analysis, tips = c('Hodgesia_cyptopus', 'Ficalbia_ichiromiyagii')))
...
#par(mai = c(2,2,2,2))
#pdf('sym.FicHod.mcc.tact_071421.pdf', width=8 ,height=8)
...
arc.cladelabels(text = 'F', node = findMRCA(tree.analysis, tips = filter(dat.for.plot, tribe.label == 'Culisetini') %>% 
                                              pull(genus.species)), mark.node = F)
arc.cladelabels(text = 'E', node = findMRCA(tree.analysis, tips = filter(dat.for.plot, tribe.label == 'Mansoniini') %>% 
                                              pull(genus.species)), mark.node = F)
...
arc.cladelabels(text = 'Culicini', node = findMRCA(tree.analysis, tips = filter(dat.for.plot, tribe.label == 'Culicini') %>% 
                                                     pull(genus.species)), mark.node = F)
arc.cladelabels(text = 'D', node = findMRCA(tree.analysis, tips = filter(dat.for.plot, tribe.label == 'Ficalbiini') %>% 
                                                     pull(genus.species)), mark.node = F)
add.arrow(tree = tree.analysis, tip = 'Hodgesia_cyptopus', col = 'black', lwd =1.5, arrl = 15)
...
#par(mai = c(2,2,2,2))
#pdf('er.FicHod.mcc.tact_071421.pdf', width=8 ,height=8)
plotTree(tree.analysis, ftype = 'off', type = 'fan', part = 0.97, lwd = 1.5)
for(i in nrow(circ.leg$leg):1){
  color<-paste(strsplit(circ.leg$colors[i],'')[[1]][1:7], collapse='')
  draw.circle(0,0, radius=r[i], col=color, border='transparent')
...
#par(mai = c(2,2,2,2))
#pdf('ard.FicHod.mcc.tact_071421.pdf', width=8 ,height=8)
plotTree(tree.analysis, ftype = 'off', type = 'fan', part = 0.97, lwd = 1.5)
for(i in nrow(circ.leg$leg):1){
  color<-paste(strsplit(circ.leg$colors[i],'')[[1]][1:7], collapse='')
  draw.circle(0,0, radius=r[i], col=color, border='transparent')
